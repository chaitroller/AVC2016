...
//Warning detection
const int distanceMMDetectInFront = 3500; // about 10 feet
const int distanceMMDetectOnSides = 600; // about 2 feet
            ...
            for (int ix = 0; ix < N_DATA_QUADS; ix++) {
              if (xv_config.aryAngles[startingAngle + ix]) {             // if we're supposed to display that angle
                if (aryInvalidDataFlag[ix] & BAD_DATA_MASK) {  // if LIDAR reported a data error...
                  ...
                  showDistance(startingAngle + ix, int(aryDist[ix]));
                }
...
//detect Object, and only write packet IF alarming or within a range
void showDistance(uint16_t angle, int distanceMM)
{
  //only look every 10' and not behind
  if ( (angle%10)>0 || ((angle>270) && (angle<360))){
    return;
  }

  bool objectdetected=false;
  
  /*  ~304mm in 1 foot.  Default threshold should be 2 feet clearance on each side
   * LOGIC:
   *      Look left, right, forward, behind at 15' angles.  Alarm only if object is within range
   *      Write Serial data:  OO{#},DD,{#} --> Angle in Radians and Distance in mm
   *      Examples:
   *        10o'clock --> LL,135,DD,
   *      Forward Distance should be different
   */

    //Forward
    if(angle>90 && angle < 180 && distanceMM<distanceMMDetectInFront){
      objectdetected=true;
    }   
    //Right, left
    else if(distanceMM<distanceMMDetectOnSides){
      objectdetected=true;
    }
