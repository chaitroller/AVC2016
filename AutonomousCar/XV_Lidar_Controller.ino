...
void setupLidar() {
  ...
  for (ixPacket = 0; ixPacket < PACKET_LENGTH; ixPacket++)  // Initialize
    Packet[ixPacket] = 0;
  ixPacket = 0;
//  Timer3.pwm(xv_config.motor_pwm_pin, 500);
//  Serial.print("Motor PWM Pin ");
//  Serial.println(xv_config.motor_pwm_pin);
  Serial.println("2. exit Setup ");
}
...
void readLidar() {
  ...
  if (Serial3.available() > 0) {                  // read byte from LIDAR and relay to USB
    inByte = Serial3.read();                      // get incoming byte:
    Serial.print("-- Serial3 got data ---- ");
            ...
            for (int ix = 0; ix < N_DATA_QUADS; ix++) {
              if (xv_config.aryAngles[startingAngle + ix]) {             // if we're supposed to display that angle
                if (aryInvalidDataFlag[ix] & BAD_DATA_MASK) {  // if LIDAR reported a data error...
                  ...
                  int lAngle = startingAngle + ix;
                  int lDistance = int(aryDist[ix]*0.0393701)/12;
                  //if ( gAngle > (180 - 45) && gAngle < (180 + 45) ) {
                    gAngle = lAngle;
                    gDistance = lDistance;
                    //Serial.print(F("A,"));                    
                    //Serial.print(gAngle);                   
